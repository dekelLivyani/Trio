...
export const boardStore = {
   ...
   mutations: {
      ...
      removeComment(state, { commentId, card, groupId }) {
         ...
         state.currBoard.groups[groupIdx].cards[cardIdx].comments.splice(commentIdx, 1)
      }
   },
   actions: {
      //Board
      ...
      async removeComment({commit}, { commentId, card, groupId, boardId }) {
         try {
            ...
            commit({type:'removeComment', commentId, card, groupId })
            return savedCard
         }
         catch (err) {
            console.log('Cannot remove board ', boardId, ',', err);
